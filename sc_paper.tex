...
Formulas are written as Ruby classes and follow a simple structure.
Formulas provide a domain specific language with the support of a full
programming language. The structure of a formula is the same as a Ruby
class. Here is a bare minimum example:
...
\texttt{example-1.2.3.tar.gz}, check its md5 sum, extract it to
\texttt{/sw/xk7/example/1.2.3/gnu4.3.4/source}, switch to that working
directory, and run the commands specified via the \texttt{system} method. Namely:
...
on the formula writing manpage located at
...
\subsubsection{\texttt{modulecommands}}
