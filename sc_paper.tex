...
based on an SWTools formatted target. Targets are formatted as
\texttt{name/version/build\_name} and correspond to the directory structure used
to install the software on the filesystem. Formulas manipulate the programming
environment and load or unload modules as appropriate. Formulas can express
dependency requirements, and even generate modulefiles for newly installed
packages.

For example, when installing new packages, a target can be specified such
as:
...
running SLES 11.1 with AMD Opteron CPUs. For the above target the directory
...
build instructions for dissimilar platforms in a single file. See Figure 1. for
the complete \texttt{python\_numpy} formula.
