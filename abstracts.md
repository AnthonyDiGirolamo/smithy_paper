...
heavily from its predecessor SWTools and the homebrew package management system
...
prone.  Smithy improves upon SWTools by providing a simpler command line
interface, modulefile generation and management, and installations via formulas
written in Ruby.  The goal of installation formulas is to consolidate all
knowledge required to build a software package on any system into a single file
which is stored in version control.  This can include: defining dependencies,
...
defining a modulefile.  Together NCCS staff have written over 150 formulas that
support both Cray and traditional clustered systems.  We hope other centers can
make use of Smithy and contribute their own formulas as well.


150 Words:

Smithy is a software compilation and installation tool that borrows ideas heavily from its predecessor SWTools and the Homebrew package management system for Mac OS X.  Smithy is designed to manage software builds within an HPC Linux environment using modulefiles to load software into a user's shell.  SWTools has set very good conventions for software installations at the NCCS.  Smithy improves upon SWTools by providing a simpler command line interface, modulefile generation, and installations via formulas written in Ruby.  The goal of formulas is to consolidate all knowledge required to build a software package on any system into a single file.  This can include: defining dependencies, loading modules, setting environment variables, applying patches, creating makefiles, compilation, tests, and defining a modulefile.  Together NCCS staff have written 150 formulas that support Cray and clustered systems.  We hope other centers can make use of Smithy and contribute their own formulas as well.
